...
class BatchProcessChecker:
    ...
    def load_and_validate_sample(self, sample_path):
        ...
        try:
            ...
            required_fields = ['keypoints', 'traj_2d', 'valid_steps', 'image_path', 'frame_index']
            ...
            if traj.ndim != 3 or traj.shape[0] != len(keypoints) or traj.shape[2] != 2:
                self.log_warning(f"Invalid trajectory shape in {sample_path.name}: {traj.shape}")
                return None
    ...
    def create_visualization(self, image_path, data, output_path, video_name, frame_idx):
        ...
        try:
            ...
            # Generate distinct colors for each keypoint using rainbow colormap
            ...
            for i, kp in enumerate(keypoints):
                ax1.scatter(kp[0], kp[1], c=[colors[i]], s=80, alpha=0.9, edgecolors='white', linewidths=1.5)

            ax1.set_xlim(0, w)
            ax1.set_ylim(h, 0)  # Flip y-axis for image coordinates
            ax1.axis('off')

            # Plot 2: Trajectories overlay
            ax2.imshow(image)
            ...
            for i, (kp, trajectory) in enumerate(zip(keypoints, traj)):
                ...
                # trajectory shape is [T, 2] where T is number of time steps
                ...
                if hasattr(data['valid_steps'], 'shape') and data['valid_steps'].shape and data['valid_steps'].dtype == bool:
                    # Use boolean mask - need to match trajectory length
                    valid_mask_steps = data['valid_steps']
                    # Trim mask to match trajectory length
                    T = trajectory.shape[0]
                    valid_mask_steps = valid_mask_steps[:T]
                    valid_traj = trajectory[valid_mask_steps]
                else:
                    # Use as count
                    valid_traj = trajectory[:valid_steps]

                # Filter out invalid points (marked with -inf)
                if len(valid_traj) > 0:
                    valid_mask = ~np.isinf(valid_traj).any(axis=1)
                    if np.any(valid_mask):
                        valid_traj_filtered = valid_traj[valid_mask]

                        if len(valid_traj_filtered) > 1:
                            # Plot starting keypoint as circle
                            ax2.scatter(kp[0], kp[1], c=[color], s=80, alpha=0.9,
                                      edgecolors='white', linewidths=1.5, marker='o', zorder=3)

                            # Plot trajectory line
                            ax2.plot(valid_traj_filtered[:, 0], valid_traj_filtered[:, 1],
                                   color=color, alpha=0.7, linewidth=2)

                            # Mark each trajectory point with "x"
                            ax2.scatter(valid_traj_filtered[:, 0], valid_traj_filtered[:, 1],
                                      c=[color] * len(valid_traj_filtered), s=40, alpha=0.8, marker='x', linewidths=2)
