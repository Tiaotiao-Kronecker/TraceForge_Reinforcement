...
DEFAULT_DATASET_ADAPTER = "file_layout"
DEFAULT_XPERIENCE_EPISODE_GLOB = "*/*"
DEFAULT_XPERIENCE_WINDOW_SIZE = 512


def _build_dispatch_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Unified TraceForge batch inference entrypoint")
    parser.add_argument(
        "--dataset_adapter",
        type=str,
        required=True,
        choices=["file_layout", "xperience"],
        help="Dataset adapter to run. file_layout forwards to the maintained press-one-button runner; xperience reads raw Xperience episodes directly.",
    )
    return parser


def _forward_file_layout(argv: list[str]) -> int:
    script_path = Path(__file__).with_name("batch_infer_press_one_button_demo.py")
    cmd = [sys.executable, str(script_path), *argv]
    logger.info("Forwarding to file-layout batch runner: " + " ".join(cmd))
    completed = subprocess.run(cmd, check=False)
    return int(completed.returncode)


def _build_xperience_parser() -> argparse.ArgumentParser:
...
def _finalize_xperience_args(args: argparse.Namespace) -> argparse.Namespace:
    if args.fps <= 0:
...
def _find_xperience_episode_dirs(
    dataset_root: Path,
    *,
    episode_glob: str,
...
def _resolve_xperience_frame_count(handle: h5py.File) -> int:
    return int(
        min(
...
def _build_xperience_video_name(episode_dir: Path, camera_name: str, start_index: int, stop_index: int) -> str:
    return f"{episode_dir.parent.name}__{episode_dir.name}__{camera_name}__{start_index:06d}_{stop_index:06d}"


def _load_xperience_scene_bundle(
    episode_dir: Path,
    *,
    dataset_root: Path,
    ...
    return {
        ...
        "scene_id": _build_xperience_video_name(episode_dir, args.camera_name, start_index, stop_index),
...
def _write_xperience_language_file(output_root: Path, video_name: str, language_text: str) -> None:
...
def _run_xperience(argv: list[str]) -> int:
    parser = _build_xperience_parser()
    try:
        args = _finalize_xperience_args(parser.parse_args(argv))
    ...
    episode_dirs = _find_xperience_episode_dirs(
        dataset_root,
    ...
    try:
        for episode_dir in episode_dirs:
            with h5py.File(episode_dir / "annotation.hdf5", "r") as handle:
                frame_count = _resolve_xperience_frame_count(handle)
            ...
            for window_start, window_stop in windows:
                video_name = _build_xperience_video_name(episode_dir, args.camera_name, window_start, window_stop)
                ...
                try:
                    ...
                    scene_bundle = _load_xperience_scene_bundle(
                        episode_dir,
                        dataset_root=dataset_root,
                        args=args,
                    ...
                    _write_xperience_language_file(out_dir, video_name, str(scene_bundle.get("language_text") or ""))
                    processed += 1
...
def main(argv: list[str] | None = None) -> None:
    ...
    if dispatch_args.dataset_adapter == "file_layout":
        raise SystemExit(_forward_file_layout(remaining))
    if dispatch_args.dataset_adapter == "xperience":
        raise SystemExit(_run_xperience(remaining))
    raise SystemExit(f"Unsupported dataset_adapter={dispatch_args.dataset_adapter!r}")
